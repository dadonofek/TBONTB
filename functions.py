...
def simulate_investment(initial_fortune, monthly_contribution, years, tax_rate,
    ...
    for path in range(n_sim):
        ...
        balance = initial_fortune if initial_already_invested else initial_fortune * (1 - transaction_fee / 100)
        total_deposits = initial_fortune
        monthly_path = []
        monthly_path.append(balance)

        for m in range(1, months + 1):
            year_idx = (m - 1) // 12
            month_idx = (m - 1) % 12
            current_contribution = contributions_schedule[year_idx][month_idx]
            balance, total_deposits = update_monthly_balance(balance, total_deposits, current_contribution,
                                                                  forecast_params)
            monthly_path.append(balance)
